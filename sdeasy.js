$(function () {
    ...
    function error() {
        alert(
            "There was a problem and the action could not be processed. Please try again later."
        );
    }
    ...
    $.fn.message = function (obj) {
        ...
        var msg = $(
            `<div class='alert ${obj.type} alert-${obj.type} alert-dismissible' style="display: none" role="alert">${obj.message}<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>`
        );
        this.append(msg);
        let readingTimeRemaining =
            ((msg.text().length * 60) / 24) * 0.13 * 100 + 2000;
        msg
            .slideDown()
            .delay(readingTimeRemaining)
        ...
        if (current.attr("datatable-ajax")) {
            config.ajax = {
                ...
                beforeSend: function () {
                    ...
                    current
                        .children("tbody")
                        .html('<tr><td colspan="122" class="spinner p-4"></td></tr>');
                }
            };
        }
                    ...
                    html: `<div class="d-flex justify-content-between">
                          ...
                          <span>
                              ...
                              <small class="text-muted d-flex justify-content-between">
                                  <span class="ps-1">${(data.size * 1.0).toFixed(
                        1
                    )}</span>
                                  <span class="pe-1 ms-3">${(
                            data.capacity * 1.0
                        ).toFixed(1)}</span>
                              </small>
                          </span>
    ...
    $.fn.sdajax = function (
        ...
        if (redirect) redirect = redirect.split(":");
        let settings = {};
        settings.async = true;
        ...
        settings.success = function (data) {
            ...
            if (insertTo && data.type == "success" && !autoUpdate) {
                ...
                else $(data.message).appendTo(insertTo).fadeIn("slow");
            } else if (content.length && xhrDataType == "html") {
                content.scroll();
                content.html(data);
            } else if (content.length && data.message) {
                if (xhrDataType == "html") content.html(data.message);
                else {
            ...
            if (data.type == "success") {
                if (modalDispose) $(modalDispose).modal("hide");
                ...
                if (data.message.terminal) window.location.href = data.message.terminal;
                if (autoUpdate) {
                    const target = $this.data("parent");
                    if (autoUpdateType == "one") $this.data("auto-update", "false");
                    for (const [key, value] of Object.entries(data.message)) {
                        ...
                        if (jsonDomItem.length) {
                            ...
                            if (typeof then !== "undefined") {
                                if (data.message[then]) jsonDomItem.show();
                                else jsonDomItem.hide();
                            }
                            switch (updateType) {
                                case "img":
                ...
                if (data.scrollTo) $(window).scrollTop($(data.scrollTo).offset().top);
                if (injectCode) eval(injectCode);
                if (redirect)
                    setTimeout(function () {
                        window.location.href = redirect[0];
                    }, redirect[1]);
            }
            onAjaxDone === null || onAjaxDone === void 0 || onAjaxDone(data);
    ...
    $("body").on("click", "[href]", function () {
        if (window.location.pathname == $(this).attr("href")) return false;
    });
    ...
    $("body").on("click", "[data-url], [live='true']", function () {
        ...
        if (!actionDataType) actionDataType = "json";
        let spinner = $this.attr("spinner");
        if (typeof spinner !== "undefined") {
            if (spinner === "false") spinner = null;
        ...
        $this.sdajax(
            spinner,
            function (data) {
                if (hasCountable) $this.data("countable", num);
                if (live) {
    ...
    $("body").on("submit", '[role="form"]', function (event) {
        if (!event.isDefaultPrevented()) {
            ...
            if (beforeFunc) eval(beforeFunc);
            let button = null;
            if (event.hasOwnProperty("originalEvent"))
                button = $(event.originalEvent.submitter);
            else button = event.handleObj.handler.arguments[1].submitter;
            ...
            $this.sdajax(
                ...
                action,
                new FormData(this) /*$(this).serialize()*/,
                method
            );
            return false;
        }
    ...
    $(document).on("show.bs.modal", ".modal", (event) => {
        ...
        const privateData = modal.data("private");
        if (privateData) {
            if (!event.relatedTarget) target = $(document.createElement("tempdata"));
            target.data("json", privateData.message.data);
            target.data("select-name", privateData.message.selectName);
            target.data("select", privateData.message.selectData);
            target.data("action", privateData.message.action);
        }
        var trigger = target.data("trigger");
        if (trigger) $(trigger).trigger(target.data("trigger-type"));
        var dataRun = target.data("run");
        if (dataRun) eval(dataRun);
        if (title) titleContent.html(title);
        const action = target.data("action");
        if (action) {
            const form = modal.find("form");
            const datatableAction = target.attr("datatable-action");
            if (datatableAction) form.attr("datatable-action", datatableAction);
            const dataTableName = target.attr("datatable-name");
            if (dataTableName) form.attr("datatable", dataTableName);
            ...
            if (selectize.length) selectize.selectize()[0].selectize.setValue(0);
            ...
            const step2 = function () {
                ...
                if (lazyData.length) {
                    ...
                    lazyData.each(function () {
                        const $this = $(this);
                        if (
                            target.data("select-name") == $this.attr("name") &&
    ...
    $("input[data-input-toggle]").on("change", function () {
        ...
        if ($(this).prop("checked")) $(value).slideDown("fast");
        else $(value).slideUp("fast");
    });
    ...
    $(document).on("change", "[data-fill]", function () {
        ...
        data.forEach((element, i) => {
            $(element).sdajax(
                ...
                function (data) {
                    const obj = $(element);
                    if (!obj.attr("select2")) obj.html(data);
                    else {
